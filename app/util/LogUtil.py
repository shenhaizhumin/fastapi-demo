...
project_dir = file_path[: file_path.rfind("/server/")]
...
class LogUtil(object):
    ...
    conf = {
        ...
        # "error_sort": True,
        # "error_filename": "{dir}/log/app.test.error.log".format(dir=project_dir),
    }
    ...
    def init_logger(self):
        ...
        if self.conf.get('error_sort'):
            handler = self.getNoRotatingErrorFileHandler()
            self.conf["logger"].addHandler(handler)

        if self.conf.get("debug"):
            stream_handler = logging.StreamHandler()
            stream_handler.setFormatter(logging.Formatter(self.conf["format"]))
    ...
    def getTimedRotatingFileHandler(self):
        ...
        handler.setFormatter(logging.Formatter(self.conf["format"]))
        return handler
    ...
    def getNoRotatingFileHandler(self):
        ...
        获取无轮转文件的handler
        """
        handler = logging.FileHandler(filename=self.conf["filename"])
        handler.setFormatter(logging.Formatter(self.conf["format"]))
        return handler

    def getNoRotatingErrorFileHandler(self):
        """
        获取无轮转文件的handler
        """
        handler = logging.FileHandler(filename=self.conf["error_filename"])
        handler.setFormatter(logging.Formatter(self.conf["format"]))
